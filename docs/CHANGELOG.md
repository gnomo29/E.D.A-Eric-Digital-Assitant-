...
Se aplicaron optimizaciones de estabilidad sin cambiar funcionalidades: robustez de TTS en Windows (COM por hilo y cierre más limpio), caché temporal de `health_check`, y cierre explícito de conexiones SQLite en módulos de memoria/triggers. Además, se corrigió un ruido de logging en `remote_llm` y se ajustó `bootstrap_v3 --no-tests` para no bloquear en validación de integridad cuando el entorno local está incompleto (se reporta como warning).

Se añadió una capa de operación local: `EDA_OFFLINE_MODE` para bloquear búsquedas web/remotas, comando de trazabilidad `que hiciste` para consultar la última acción operativa, y memoria contextual por proyecto (`guardar/mostrar contexto proyecto ...`). También se incorporó cobertura de pruebas para estos flujos.
